...
d3.json('assets/theme/data/map.geojson').then(function(geojson) {
    ...
    ecowas.selectAll("g").data(geojson.features).enter().append('g').attr("class",function(d){return d.properties.name}).attr("fill",function(d,i){return couleur2[i]}).attr("stroke", "white").attr("stroke-width", "0").append("path").attr("d", path).attr("class", 'path')
    ...
    d3.select(this)
      ...
      var mouseleave2 = function(d) {
        ...
        d3.selectAll('.bar').on("mouseleave", mouseleave2);
            ...
            d3.select("#som2").append("text").attr("x",95).attr('y',23).text(total2).attr('class', 'coul2').style('font-size', "30px").style('font-family', 'sans-serif, arial')
            d3.select("#moy1").append("text").attr("x",90).attr('y',23).text(parseInt(moy)).attr('class', 'coul1').style('font-size', "30px").style('font-family', 'sans-serif, arial')
            d3.select("#moy2").append("text").attr("x",110).attr('y',23).text(parseInt(moy1)).attr('class', 'coul2').style('font-size', "30px").style('font-family', 'sans-serif, arial')
...
d3.csv("assets/theme/data/annees.csv")
    ...
    .then((data) => {
        //Abscisse
        var moyenne=d3.mean(data, function(d) {return (d["Malades"]);})
        var variance=d3.variance(data, function(d) {return (d["Malades"]);})
        var moyenne1=d3.mean(data, function(d) {return (d["Deces"]);})
        var variance1=d3.variance(data, function(d) {return (d["Deces"]);})
        ...
        .domain([-1*d3.max(data, function(d) { return (d.Malades - moyenne)/variance; }), d3.max(data, function(d) { return (d.Malades - moyenne)/variance; })])
        .range([ 200, 0]);
        svg5.append("g")
        ...
        .enter()
        .append("rect")
        .attr("x", function(d) { return absc(d.Annees); })
        .attr("y", function(d) { 
          if(((d.Malades - moyenne)/variance)>0){
            return ord((d.Malades - moyenne)/variance)
          }
          else{
            return height/2
          }
        ...
        .attr('height', function(d) {
          if(((d.Malades - moyenne)/variance)>0){
            return height/2 - ord((d.Malades - moyenne)/variance)
          }
          else{
            return ord((d.Malades - moyenne)/variance) - height/2
          }})
        .attr("r", "6")
        .style("fill", "orange")
        .attr("stroke", "black")
        ...
        .domain([-1*d3.max(data, function(d) { return (d.Deces - moyenne1)/variance1; }), d3.max(data, function(d) { return (d.Deces - moyenne1)/variance1; })])
        .range([ 200, 0]);
        svg6.append("g")
        ...
        svg5.append('rect').attr('y',220).attr('x', -30).attr('width', 30).attr('height', 5).attr('fill', 'orange')
        var txt=svg5.append('text').data(data)
        txt.attr('class', 'texte')
        .attr('x', 10).attr('y', 227)
        .text(function(){return 'CEDEAO: '})
        .style('fill', 'white').style('stroke-width', 1).style('font-size', '12px')
        // Lines
        ...
        .enter()
        .append("rect")
        .attr("x", function(d) { return absc1(d.Annees); })
        .attr("y", function(d) { 
          if(((d.Deces - moyenne1)/variance1)>0){
            return ord1((d.Deces - moyenne1)/variance1)
          }
          else{
            return height/2
          }
        ...
        .attr('height', function(d) {
          if(((d.Deces - moyenne1)/variance1)>0){
            return height/2 - ord1((d.Deces - moyenne1)/variance1)
          }
          else{
            return ord1((d.Deces - moyenne1)/variance1) - height/2
          }})
        .attr("r", "6")
        .style("fill", "red")
        .attr("stroke", "black")
        ...
        svg6.append('rect').attr('y',220).attr('x', -30).attr('width', 30).attr('height', 5).attr('fill', 'red')
        var txt1=svg6.append('text').data(data)
        ...
        d3.selectAll('.cercles').on('mousemove', function(d, i){
            //d3.select(this).on('mousemove', function(){
                ...
                txt.attr('class', 'texte')
                .attr('x', 10).attr('y', 227)
                .text(function(){return 'CEDEAO: '+d.Malades+' cas en '+d.Annees})
                .style('fill', 'white').style('stroke-width', 1).style('font-size', '12px')
            //})
        });
        ...
        d3.selectAll('.cercles1').on('mousemove', function(d, i){
            //d3.select(this).on('mousemove', function(){
                ...
                txt1.attr('class', 'texte')
                .attr('x', 10).attr('y', 227)
                .text(function(){return 'CEDEAO: '+d.Deces+' décès en '+d.Annees})
                .style('fill', 'white').style('stroke-width', 1).style('font-size', '12px')
            //})
        });
...
var vise=document.getElementById('serere').selectedOptions[0].text
var vise2 = 'assets/theme/data/'+vise+'.csv'
var mape = 'assets/theme/data/'+vise+'.geojson'
  ...
  var mouseleave1 = function(d) {
    ...
    d3.select(this)
      ...
      var mouseleave2 = function(d) {
        ...
        .selectAll("text")
        .attr("transform", "translate(-10,0)rotate(-45)")
        .style("text-anchor", "end").style('display', 'none');
        svg6.append("g")
        .attr("transform", "translate(0," + 100 + ")")
        .call(d3.axisBottom(absc1))
        .selectAll("text")
        ...
        .call(d3.axisLeft(ord))
        .selectAll("text")
        .style("text-anchor", "end").style('font-size', "8px").style('display', 'none');
        svg6.append("g")
        .call(d3.axisLeft(ord1))
        .selectAll("text")
        .style("text-anchor", "end").style('font-size', "8px").style('display', 'none');
        ...
        .attr("stroke", "black")
        .attr('class', 'cercle4')
        svg5.append('rect').attr('y',275).attr('x', -30).attr('width', 30).attr('height', 5).attr('fill', 'pink')
        var txt=svg5.append('text').data(data)
        txt.attr('class', 'texte')
        .attr('x', 10).attr('y', 281)
        .text(function(d){return d.Country+':'})
        .style('fill', 'white').style('stroke-width', 1).style('font-size', '12px')
        //var texte=svg5.append("text").data(data)
        //var texte2=svg6.append("text").data(data)
        d3.selectAll('.cercle4').on('mousemove', function(d, i){
            d3.select(this).on('mousemove', function(){       
                d3.select(this).style('fill', 'white')       
                txt.attr('class', 'texte')
                .attr('x', 10).attr('y', 281)
                .text(function(){return d.Country+': '+d.Malades+' cas en '+d.Annees})
                .style('fill', 'white').style('stroke-width', 1).style('font-size', '12px')
          })
        });
        d3.selectAll('.cercle4').on('mouseleave', function(d){
        ...
        .attr('height', function(d) {
          if(((d.Deces - moyenne1)/variance1)>0){
            ...
            return (ord1((d.Deces - moyenne1)/variance1) - height/2)/2
          }})
        .attr("r", "6")
        .style("fill", "pink")
        .attr("stroke", "black")
        .attr('class', 'cercle3')
        svg6.append('rect').attr('y',275).attr('x', -30).attr('width', 30).attr('height', 5).attr('fill', 'pink')
        var txt1=svg6.append('text').data(data)
        txt1.attr('class', 'texte')
        .attr('x', 10).attr('y', 281)
        .text(function(d){return d.Country})
        ...
        //var texte=svg5.append("text").data(data)
        //var texte2=svg6.append("text").data(data)
        d3.selectAll('.cercle3').on('mousemove', function(d, i){
            d3.select(this).on('mousemove', function(){      
                d3.select(this).style('fill', 'white')       
                txt1.attr('class', 'texte')
                .attr('x', 10).attr('y', 281)
                .text(function(){return d.Country+': '+d.Deces+' décès en '+d.Annees})
                .style('fill', 'white').style('stroke-width', 1).style('font-size', '12px')
          })
        });
        d3.selectAll('.cercle3').on('mouseleave', function(d){
          d3.select(this).style('fill', 'pink');
          //texte.text('');
        })
...
d3.csv("assets/theme/data/annees.csv")
    ...
    .then((data) => {
        ...
        .enter()
        .append("rect")
        .attr("x", function(d) { return absc(d.Annees)+7; })
        .attr("y", function(d) { 
          if(((d.Malades - moyenne)/variance)>0){
            return ord((d.Malades - moyenne)/variance)
          }
        ...
        svg5.append('rect').attr('y',250).attr('x', -30).attr('width', 30).attr('height', 5).attr('fill', 'orange')
        var txt2=svg5.append('text').data(data)
        txt2.attr('class', 'texte')
        .attr('x', 10).attr('y', 255)
        .text(function(){return 'CEDEAO: '})
        .style('fill', 'white').style('stroke-width', 1).style('font-size', '12px')
        svg6.append('rect').attr('y',250).attr('x', -30).attr('width', 30).attr('height', 5).attr('fill', 'red')
        var txt3=svg6.append('text').data(data)
        txt3.attr('class', 'texte')
        .attr('x', 10).attr('y', 255)
        .text(function(){return 'CEDEAO: '})
        .style('fill', 'white').style('stroke-width', 1).style('font-size', '12px')
        //var texte=svg5.append("text").data(data)
        //var texte2=svg6.append("text").data(data)
        d3.selectAll('.cercler').on('mousemove', function(d, i){
            //d3.select(this).on('mousemove', function(){
                d3.select(this).style('fill', 'white')       
                txt2.attr('class', 'texte')
                .attr('x', 10).attr('y', 255)
        ...
        d3.selectAll('.cercler').on('mouseleave', function(d){
          d3.select(this).style('fill', 'orange');
          texte.text('');
        })
        //ziyoscxsxssklhck
        d3.selectAll('.cercle1').on('mousemove', function(d, i){
            //d3.select(this).on('mousemove', function(){
                d3.select(this).style('fill', 'white')       
                txt3.attr('class', 'texte')
                .attr('x', 10).attr('y', 255)
                .text(function(){return 'CEDEAO: '+d.Malades+' décès en '+d.Annees})
                .style('fill', 'white').style('stroke-width', 1).style('font-size', '12px')
            //})
        });
        d3.selectAll('.cercle1').on('mouseleave', function(d){
          d3.select(this).style('fill', 'orange');
          texte.text('');
        })
        ...
        .enter()
        .append("rect")
        .attr("x", function(d) { return absc1(d.Annees)+7; })
        .attr("y", function(d) { 
          if(((d.Deces - moyenne1)/variance1)>0){
            return ord1((d.Deces - moyenne1)/variance1)
          }
        ...
        var texte=svg5.append("text").data(data)
        //var texte2=svg6.append("text").data(data)
        d3.selectAll('.cercle2').on('mousemove', function(d, i){
            //d3.select(this).on('mousemove', function(){
                d3.select(this).style('fill', 'white')       
                txt3.attr('class', 'texte')
                .attr('x', 10).attr('y', 255)
        ...
        d3.selectAll('.cercle2').on('mouseleave', function(d){
          d3.select(this).style('fill', 'red');
          //texte.text('');
        })
    })

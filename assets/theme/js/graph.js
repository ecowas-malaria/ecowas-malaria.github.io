...
d3.csv("assets/theme/data/annees.csv")
    ...
    .then((data) => {
        ...
        .attr("y", function(d) { return ord1(d.Deces); })
        .attr('width', x1.bandwidth())
        .attr('height', function(d) { return height-ord1(d.Deces)})
        .attr("r", "6")
        .style("fill", "red")
        .attr("stroke", "black")
        .attr('class', 'cercles1')
...
d3.csv(vise2)
  ...
  var mouseleave1 = function(d) {
    ...
    var moyenne=d3.mean(data, function(d) {return (d["Malades"]);})
        ...
        var texte=svg5.append("text").data(data)
        //var texte2=svg6.append("text").data(data)
        d3.selectAll('.cercle').on('mousemove', function(d, i){
            //d3.select(this).on('mousemove', function(){
                d3.select(this).style('fill', 'white')       
                texte.attr('class', 'texte')
                .attr('x', function(){return absc(d.Annees)}).attr('y', function(){return ord(d.Malades)})
        ...
        d3.selectAll('.cercle').on('mouseleave', function(d){
...
d3.csv("assets/theme/data/annees.csv")
    ...
    .then((data) => {
        ...
        .enter()
        .append("rect")
        .attr("x", function(d) { return absc(d.Annees)+x1.bandwidth(); })
        .attr("y", function(d) { 
          if(((d.Malades - moyenne)/variance)>0){
            return ord((d.Malades - moyenne)/variance)
          }
...
d3.csv("assets/theme/data/2017.csv")
    ...
    .then((data) => {
      ...
      //ar cercle=c1.selectAll('.circle1').data(data).enter().append('circle').attr('class', 'circle1')
